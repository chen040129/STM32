...
typedef struct
{
    uint32_t CardType;        /*!< 指定卡的类型，表示SD卡的类型（SDSC、SDHC、SDXC等） */

    uint32_t CardVersion;     /*!< 指定卡的版本，表示SD卡的版本信息 */

    uint32_t Class;           /*!< 指定卡的类别，表示SD卡的速度类别 */

    uint32_t RelCardAdd;      /*!< 指定卡的相对地址，表示SD卡的相对地址 */

    uint32_t BlockNbr;        /*!< 指定卡的容量，表示SD卡的总块数 */

    uint32_t BlockSize;       /*!< 指定块的大小，表示SD卡的块大小 */

    uint32_t LogBlockNbr;     /*!< 指定卡的逻辑容量，表示SD卡的逻辑块数 */

    uint32_t LogBlockSize;    /*!< 指定逻辑块的大小，表示SD卡的逻辑块大小 */
...
typedef struct
{
    __IO uint8_t  ManufacturerID;  /*!< 制造商ID */
    __IO uint16_t OEM_AppliID;     /*!< OEM/Application ID，原始设备制造商或应用程序ID */
    __IO uint32_t ProdName1;       /*!< 产品名称的第一部分 */
    __IO uint8_t  ProdName2;       /*!< 产品名称的第二部分 */
    __IO uint8_t  ProdRev;         /*!< 产品版本号 */
    __IO uint32_t ProdSN;          /*!< 产品序列号，唯一标识一个特定的SD卡 */
    __IO uint8_t  Reserved1;       /*!< 保留字段1 */
    __IO uint16_t ManufactDate;    /*!< 制造日期 */
    __IO uint8_t  CID_CRC;         /*!< CID寄存器的CRC校验值 */
    __IO uint8_t  Reserved2;       /*!< 保留字段2，通常为1 */
